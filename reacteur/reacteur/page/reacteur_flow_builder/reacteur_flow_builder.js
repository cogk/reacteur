const PAGE_NAME = "reacteur-flow-builder";

frappe.pages[PAGE_NAME].on_page_load = function (wrapper) {
	frappe.ui.make_app_page({
		parent: wrapper,
		title: __("Reacteur Flow Builder"),
		single_column: true,
	});
	...
	if (frappe.boot.developer_mode) {
		...
		frappe.hot_update.push(() => load_reacteur_builder(wrapper));
	}
};

frappe.pages[PAGE_NAME].on_page_show = function (wrapper) {
	load_reacteur_builder(wrapper);
};

function load_reacteur_builder(wrapper) {
	...
	if (route.length > 1) {
		frappe.require("workflow_builder.bundle.js").then(() => {
			reacteur.reacteur_flow_builder = new reacteur.ReacterFlowBuilder({
				wrapper: $parent,
				page: wrapper.page,
				workflow: route[1],
			});
		...
		let d = new frappe.ui.Dialog({
			...
			primary_action({ action, doctype, workflow, workflow_name }) {
				if (action === "Edit") {
					frappe.set_route(PAGE_NAME, workflow);
					...
					frappe.db
						...
						.then((doc) => {
							frappe.set_route(PAGE_NAME, doc.name);
						})
						.finally(() => {
